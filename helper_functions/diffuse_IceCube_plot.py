...
import os
def read_out_diffuse_HESE_data(filename="../data/icecube-dffisue/diffuse_HESE_7.5_year_differential.txt"):
    #module_dir = os.path.dirname(__file__)
    file_path = filename#os.path.join(module_dir, filename)
    f = open(file_path,"r")
    lines = f.readlines() 
...
flux,   flux_lower_err,   flux_upper_err = read_out_diffuse_HESE_data(filename="../data/icecube-diffuse/diffuse_HESE_7.5_year_differential.txt")
...
energy_numu, flux_lower = read_out_diffuse_data("../data/icecube-diffuse/diffuse_numu_9.5_year_lower_band.txt",
                                                 energy_range_numu.min(),
                                                 energy_range_numu.max(),
                                                 steps=1e3)

energy_numu, flux_upper = read_out_diffuse_data("../data/icecube-diffuse/diffuse_numu_9.5_year_upper_band.txt",
                                                 energy_range_numu.min(),
                                                 energy_range_numu.max(),
                                                 steps=1e3)
